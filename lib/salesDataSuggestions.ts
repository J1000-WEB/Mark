...
// MARK 2026-09-22: (2026-09-21에 만든 최초 버전의 컬럼 규칙이 틀렸던 걸 소천님이 실측으로
// 잡아주셔서 수정) PIP 파일은 스타일/칼라/사이즈별로 매장(오프라인 24개 + 온라인/물류 등)마다
// [일간,일간금액,주간,누적,재고] 5칸이 반복되는 구조입니다(매장명 병합헤더도 정확히 이 5칸을
// 덮습니다 — 엑셀에서 "!merges"로 직접 확인함). 매장명 라벨(헤더 2번째 줄, 0-index로
// pipRows[1])은 이 5칸 블록의 "첫 번째" 칸("일간" 위치)에 붙어있고, 그 매장 자기 자신의
// "재고" 컬럼은 그 블록의 "마지막"(라벨 위치 + 4칸 뒤)입니다.
// 처음엔 "라벨 바로 왼쪽 칸(index-1)"이 재고라고 잘못 판단했는데(그 칸도 마침 "재고"라는
// 서브라벨이 붙어있어서 그럴듯해 보였음 — 사실은 그 왼쪽 칸은 "바로 앞 매장" 블록의 재고
// 칸이었습니다), 그래서 모든 매장의 재고를 "한 매장 앞"의 값으로 읽고 있었습니다(성수 플래그십에
// 표시된 재고가 실제로는 포시즌 아울렛 신사점 것이었던 식). 실제 샘플 파일(GF3LTS503,
// 2026-09-22 업로드분)에서 소천님이 직접 확인해주신 "성수 플래그십 재고 32개"와 index+4 칸
// (엑셀 컬럼 IY)의 실제 합계가 정확히 일치하는 것으로 검증했습니다. "_"로 시작하는 매장명
// (예: "오프라인_현대신촌(교원)")은 중복/레거시 항목이라 제외하고, 매장명처럼 안 보이는 라벨
// ("제이미" 등 실제 매장이 아닌 집계열)도 정규식으로 걸러냅니다.
...
function detectPipStoreStockColumns(pipRows: any[][]): { storeName: string; stockCol: number }[] {
  ...
  for (let i = 0; i < nameRow.length; i++) {
    ...
    // "무신사"(위탁/입점 채널)는 "신사"를 포함해서 정규식에 false-positive로 걸립니다 —
    // 서버 쪽 isConsignmentChannel()과 동일하게 명시적으로 제외합니다(예: "무신사 #6 여주 2센터"
    // 같은 물류센터 채널이 "신사 플래그십"으로 오인되는 걸 방지).
    if (name.includes("무신사")) continue;
    if (!PIP_STORE_NAME_PATTERN.test(name)) continue;
    const subHere = String(subRow[i] ?? "").trim();
    const subStock = String(subRow[i + 4] ?? "").trim();
    if (subHere !== "일간" || subStock !== "재고") continue; // 라벨 위치가 예상과 다르면(포맷 변경 등) 건너뜀
    result.push({ storeName: name, stockCol: i + 4 });
  }
  return result;
}

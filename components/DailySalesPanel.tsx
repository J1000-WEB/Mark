...
function DailyStoreBriefingSection() {
  ...
  async function loadBriefing(store: string, withStores = false) {
    setLoading(true);
    setError("");
    try {
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <select
          value={selectedStore}
          onChange={(e) => { setSelectedStore(e.target.value); loadBriefing(e.target.value); }}
        ...
        <div className="mt-4 space-y-4">
          <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
            <div className="rounded-2xl bg-slate-50 p-4">
              <p className="text-xs font-bold text-slate-500">{briefing.scope} 오늘({briefing.targetDate}) 매출</p>

...
export async function GET() {
  try {
    ...
    const specialOfferResult = await buildSpecialOfferEvents(dailyFlatRows);
    const specialOfferEvents = specialOfferResult.events;
    const dailyRevenue = buildDailyRevenueSeries(dailyFlatRows);

    const allEvents = [...events, ...specialOfferEvents];

    return NextResponse.json(
      { ok: true, sheetName, events: allEvents, dailyRevenue, specialOfferDebug: specialOfferResult.debug },

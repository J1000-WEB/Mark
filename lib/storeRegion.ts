...
export function weatherActionTip(weather: { weather: string; maxTemp: number; minTemp: number; rainChance: number } | null) {
  ...
  const isHot = Number(weather.maxTemp) >= 28;
  const isCold = Number(weather.maxTemp) <= 5;

  if (isSnow) {
    return { icon: "❄️", text: "눈 예보가 있어요. 방문객이 줄 수 있어요 — 온라인/전화 문의 응대를 준비해두시고, 아우터·니트류를 입구 쪽에 배치해보세요." };
  }
  if (isRain) {
    return { icon: "☔", text: "비 예보가 있어요. 우산 매대를 입구 앞으로, 방수 소재/아우터를 전진배치하면 좋아요. 우천 시 평소보다 방문이 줄 수 있으니 온라인 채널 프로모션도 고려해보세요." };
  }
  if (isHot) {
    return { icon: "🌤️", text: "더운 날씨예요. 린넨·반팔 등 시원한 소재를 전면에, 에어컨 가까운 동선에 여름 상품을 배치해보세요." };
  }
  if (isCold) {
    return { icon: "🧥", text: "쌀쌀한 날씨예요. 아우터·니트류를 입구 쪽 눈에 띄는 자리로, 매장 입구 온기에도 신경 써주세요." };
  }
  return { icon: "🌈", text: "평년 수준의 날씨예요. 평소 진열을 유지하시면 될 것 같아요." };
}

...
function EventBar({ event, monthStartDate, monthEndDate, dayWidth }: { event: any; monthStartDate: Date; monthEndDate: Date; dayWidth: number }) {
  ...
  return (
    <div
      ...
      title={`${event.startDate}${event.endDate && event.endDate !== event.startDate ? ` ~ ${event.endDate}` : ""} / ${event.group || ""} / ${event.title}`}
    >
...
export default function ScheduleDashboard() {
  ...
  const filteredEvents = useMemo(() => {
    ...
    return events.filter((event: any) => {
      ...
      if (hiddenSchedule && event.category === "schedule" && /휴무/.test(`${event.title} ${event.group} ${event.content}`)) return false;
      if (!q) return true;
      const raw = Object.values(event.raw || {}).join(" ");
      return `${event.title} ${event.group} ${event.largeCategory} ${raw}`.toLowerCase().includes(q);
    });
            ...
            {CATEGORY_ROWS.map((row) => {
              ...
              return (
                <div key={row.key} className="flex border-b border-slate-100">
                  <div className={`sticky left-0 z-10 flex w-36 shrink-0 items-center justify-between border-r border-slate-200 bg-white p-3 ${categorySoftClass(row.key)}`}>
                    <div>
              ...
              <tbody>
                {filteredEvents.map((event: any) => (
                  <tr key={`table-${event.id}`} className="border-t border-slate-100">
                    ...
                    <td className="px-3 py-2 text-slate-600">{event.title}</td>
                  </tr>
                ))}
              </tbody>
            </table>
